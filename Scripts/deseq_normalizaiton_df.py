...
def gene_counts_control(df_count_matrix): # Remove the line which all of the gene counts less than 5; The index should be continuous from 0.
    for i in range(len(df_count_matrix)):
        flag = 1
        if sum(df_count_matrix.iloc[i, 4:50]) < 230:
            flag = 0
            for col in range(46):
                if df_count_matrix.iloc[i, col+4] >= 5:
                    flag = 1
                    break
...
def deseq_normalization(df_cnt_matrix_control):
    df_cnt_nml = df_cnt_matrix_control.loc[:, ['name2', 'name', 'chrom', 'TSS_site']]
    for i in range(len(df_cnt_matrix_control)):
        count_list = [int(x) for x in df_cnt_matrix_control.iloc[i, 4:50]]
        df_cnt_matrix_control.loc[i, 'ref_cnt'] = stats.mstats.gmean(count_list)
    for col in range(46):
        col_name = df_cnt_matrix_control.columns[col+4]
        col_factor_name = col_name + '_factor'
        col_name_nml = col_name + '_nml'
        df_cnt_matrix_control.loc[:, col_factor_name] = df_cnt_matrix_control.loc[:, col_name] / df_cnt_matrix_control.loc[:, 'ref_cnt']
        col_factor_median = np.median(df_cnt_matrix_control[col_factor_name].dropna(axis=0, how='any'))
        df_cnt_matrix_control.loc[:, col_name_nml] = np.log2(df_cnt_matrix_control.loc[:, col_name] / col_factor_median + 0.5)
        #df_cnt_nml.loc[:, col_name_nml] = df_cnt_matrix_control.loc[:, col_name_nml]
    return df_cnt_matrix_control
...
Df_cnt_nml.to_csv('D:\\G_project\\Zhen_G\\data\\gene_counts_table_nml6.txt', index=None, sep='\t')
